...
static void make_detail_line(IssTrackerApp *app, char *buf)
{
    ...
    if(app->language==ISS_LANG_EN){ if(europe==2) append_str(buf,&p,"Notice: ISS is over Europe"); else if(europe==1) append_str(buf,&p,"Notice: ISS approaching Europe"); append_str(buf,&p,"  *  Updated "); } else if(app->language==ISS_LANG_PL){ if(europe==2) append_str(buf,&p,"Info: ISS jest nad Europa"); else if(europe==1) append_str(buf,&p,"Info: ISS zbliza sie do Europy"); append_str(buf,&p,"  *  Aktual "); } else { if(europe==2) append_str(buf,&p,"Hinweis: ISS ist ueber Europa"); else if(europe==1) append_str(buf,&p,"Hinweis: ISS naehert sich Europa"); append_str(buf,&p,"  *  Aktual "); }
