...
void draw_panel(struct Window *win, IssTrackerApp *app){ struct RastPort *rp; WORD mx; WORD my; WORD mw; WORD mh; WORD by; WORD sy; WORD iy; char line[120]; rp=win->RPort; layout(win,&mx,&my,&mw,&mh,&by,&sy,&iy); SetDrMd(rp,JAM1); SetAPen(rp,0); RectFill(rp,0,by-2,win->Width-1,win->Height-1); draw_button(rp,8,by,"Update",0); draw_button(rp,72,by,"Info",0); draw_button(rp,136,by,"Quit",0); SetAPen(rp,1); make_status_line(app,line); text_at(rp,8,sy,line); make_detail_line(app,line); text_at(rp,8,iy,line); }
