...
static void make_status_line(IssTrackerApp *app, char *buf)
{
    ...
    UBYTE europe;
    p=0;
    if(!app->current.valid || app->status_page==0){ append_str(buf,&p,"Status: "); append_str(buf,&p,status_name(app->status)); if(app->current.valid){ append_str(buf,&p,"  *  Daytime: "); append_str(buf,&p,light_name(app->light_state)); append_str(buf,&p,"  *  Terrain: "); append_str(buf,&p,surf_name(app->surface_state)); } return; }
    if(app->status_page==1){ append_str(buf,&p,"Position: Lat "); append_num(buf,&p,app->current.lat_cd/100); append_str(buf,&p,"  Lon "); append_num(buf,&p,app->current.lon_cd/100); return; }
    if(app->status_page==2){ append_str(buf,&p,"ISS: Hoehe 420 km  *  Tempo 27600 km/h  *  Orbit "); append_num(buf,&p,(LONG)orbit_number(app->current.timestamp)); return; }
    europe=near_europe(app);
    if(europe==2) append_str(buf,&p,"Hinweis: derzeit ueber Europa");
    else if(europe==1) append_str(buf,&p,"Hinweis: ISS naehert sich Europa");
    else append_str(buf,&p,"Hinweis: ISS ausserhalb Europa");
    append_str(buf,&p,"  *  Aktual ");
...
void draw_panel(struct Window *win, IssTrackerApp *app){ struct RastPort *rp; WORD mx; WORD my; WORD mw; WORD mh; WORD by; WORD sy; WORD iy; char line[120]; rp=win->RPort; layout(win,&mx,&my,&mw,&mh,&by,&sy,&iy); SetDrMd(rp,JAM1); SetAPen(rp,0); RectFill(rp,0,by-2,win->Width-1,win->Height-1); draw_button(rp,8,by,"Update",0); draw_button(rp,72,by,"Info",0); draw_button(rp,136,by,"Quit",0); SetAPen(rp,1); make_status_line(app,line); text_at(rp,8,sy,line); text_at(rp,8,iy,app->info_text); }
void draw_all(struct Window *win, IssTrackerApp *app){ struct RastPort *rp; UBYTE depth; WORD mx; WORD my; WORD mw; WORD mh; WORD by; WORD sy; WORD iy; rp=win->RPort; depth=(UBYTE)win->WScreen->BitMap.Depth; layout(win,&mx,&my,&mw,&mh,&by,&sy,&iy); SetDrMd(rp,JAM1); SetAPen(rp,0); RectFill(rp,0,0,win->Width-1,win->Height-1); SetAPen(rp,1); text_at(rp,PAD_X,TITLE_Y,ISS_TITLE); draw_map(rp,depth,mx,my,mw,mh); draw_iss(rp,app,mx,my,mw,mh); draw_panel(win,app); }
