...
void draw_panel(struct Window *win, IssTrackerApp *app){ struct RastPort *rp; WORD mx; WORD my; WORD mw; WORD mh; WORD by; WORD sy; WORD iy; rp=win->RPort; layout(win,&mx,&my,&mw,&mh,&by,&sy,&iy); SetDrMd(rp,JAM1); SetAPen(rp,0); RectFill(rp,0,by-2,win->Width-1,win->Height-1); draw_button(rp,8,by,"Update",0); draw_button(rp,72,by,"Auto",app->auto_update); draw_button(rp,136,by,"Info",0); draw_button(rp,200,by,"Trail",app->show_trail); draw_button(rp,264,by,"Quit",0); SetAPen(rp,1); text_at(rp,8,sy,status_name(app->status)); if(app->current.valid){ text_at(rp,72,sy,light_name(app->light_state)); text_at(rp,152,sy,surf_name(app->surface_state)); } text_at(rp,8,iy,app->info_text); }
