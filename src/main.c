...
int main(void){ IssTrackerApp app; memset(&app,0,sizeof(app)); app.show_trail=1; app.status=ISS_STATUS_OFFLINE; app.blink=1; strcpy(app.info_text,"Press Update to fetch ISS position"); IntuitionBase=OpenLibrary("intuition.library",33); GfxBase=OpenLibrary("graphics.library",33); SocketBase=OpenLibrary("bsdsocket.library",4); if(!IntuitionBase || !GfxBase){ if(SocketBase) CloseLibrary(SocketBase); if(GfxBase) CloseLibrary(GfxBase); if(IntuitionBase) CloseLibrary(IntuitionBase); return 20; } gui_run(&app); if(SocketBase) CloseLibrary(SocketBase); if(GfxBase) CloseLibrary(GfxBase); if(IntuitionBase) CloseLibrary(IntuitionBase); return 0; }
